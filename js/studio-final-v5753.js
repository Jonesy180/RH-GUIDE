...
function rhRoundNameList(){const names=rhSavedRoundNames();return names.length?`<datalist id="rhSavedRoundNames">${names.map(n=>`<option value="${esc(n)}"></option>`).join('')}</datalist>`:''}
function rhRenderSetup(){const x=rhSetup;if(!x)return;const cars=rhEligible(x.type,x.value),included=x.entries.length,rounds=x.rounds.length,trophy=rhSetupTrophyType(x.type);$('festival').innerHTML=`<div class="rhSetupV1">
 ...
 <main class="rhSetupBodyV1">
  ...
  <section class="rhSetupPanelV1">
   ...
   ${rounds?`<div class="rhSetupRoundLabelV1">ROUND ORDER <span>(USE ARROWS TO REORDER)</span></div><div class="rhSetupRoundsV1">${x.rounds.map((r,i)=>`<div class="rhSetupRoundV1"><i>≡</i><b>${i+1}</b><input list="rhSavedRoundNames" value="${esc(r.name)}" onchange="rhRenameRound('${r.id}',this.value)"><button onclick="rhMoveRound(${i},-1)" ${i===0?'disabled':''} aria-label="Move up">↑</button><button onclick="rhMoveRound(${i},1)" ${i===rounds-1?'disabled':''} aria-label="Move down">↓</button><button onclick="rhRemoveRound('${r.id}')" aria-label="Remove round">×</button></div>`).join('')}</div>`:`<div class="rhSetupNoRoundsV1"><i>⚑</i><b>NO ROUNDS CONFIGURED</b><p>Add the races/challenges you want<br>to use for this Championship.</p></div>`}
