...
window.rhResultSummary=function(r,res){
 ...
 resultScreen(q('festival'),'RESULTS SUMMARY',res.roundName,`<section class="rhResultsSummary rhResultsSummaryFinal">
  ...
  ${records?`<div class="rhRecordCelebration rhRecordCelebrationFinal"><div class="rhRosettes">${res.championshipRecord?'<b>✹ CHAMPIONSHIP RECORD</b>':''}${res.allTime?'<b class="gold">✹ ALL-TIME RACEHUB RECORD</b>':''}</div><img src="assets/final/hubs.png" alt="Hubs"><p>Incredible driving,<br><b>well done!</b></p></div>`:''}
...
window.rhRunCarCompleteTransition=function(runId,carId){
 ...
 resultScreen(q('festival'),'CAR COMPLETE',r.name,`<section class="rhCarComplete rhCarCompleteFinal">
  ...
  ${next?`<div class="rhNextCar rhNextCarFinal"><small>NEXT CAR</small><b>${safe(carName(carById(next.carId)))}</b><span>${safe(next.round.name)}</span></div><button class="btn rhPrimaryWide" onclick="rhEnterResult('${r.id}','${next.carId}','${next.round.id}')">START NEXT CAR</button>`:''}
 </section>`);
};
...
window.rhEventResultSummary=function(e,res){
 ...
 show('event');resultScreen(q('event'),'RESULTS SUMMARY',res.roundName,`<section class="rhResultsSummary rhResultsSummaryFinal">
  ...
  ${records?`<div class="rhRecordCelebration rhRecordCelebrationFinal"><div class="rhRosettes">${res.championshipRecord?'<b>✹ EVENT RECORD</b>':''}${res.allTime?'<b class="gold">✹ ALL-TIME RACEHUB RECORD</b>':''}</div><img src="assets/final/hubs.png" alt="Hubs"><p>Incredible driving,<br><b>well done!</b></p></div>`:''}
  <button class="btn rhPrimaryWide rhResultContinueFinal" onclick="${action}"><b>${label}</b><small>${sub}</small></button>
 </section>`);
};
window.rhEventCarCompleteTransition=function(eventId,carId){
 ...
 show('event');resultScreen(q('event'),'CAR COMPLETE',e.name,`<section class="rhCarComplete rhCarCompleteFinal">
  ...
  ${next?`<div class="rhNextCar rhNextCarFinal"><small>NEXT CAR</small><b>${safe(carName(next.car))}</b><span>${safe(next.round.name)}</span></div><button class="btn rhPrimaryWide" onclick="rhEventResult('${e.id}')">START NEXT CAR</button>`:''}
 </section>`);
};
window.rhEventCompleteTransition=function(eventId){
 const e=rhSpace().customEvents.find(x=>x.id===eventId);if(!e)return;show('event');
 resultScreen(q('event'),'EVENT COMPLETE',e.name,`<section class="rhCompletionMilestone rhCompletionMilestoneFinal">
  <img src="assets/final/hubs.png" alt="Hubs">
  <div class="rhCompletionCopyFinal"><small>EVENT COMPLETE</small><h2>${safe(e.name)}</h2><p>All racers have completed the Event.</p></div>
  <button class="btn rhPrimaryWide" onclick="rhOpenEvent('${e.id}')">VIEW FINAL LEADERBOARD</button>
 </section>`);
};
...
window.rhAbout=function(){
 ...
 <section class="rhSection"><h2>RACEHUB INFORMATION</h2><div class="rhInfoRow"><span>App Version</span><b>v5.7.42</b></div><div class="rhInfoRow"><span>Data / Backup Version</span><b>v1.0.0</b></div><div class="rhInfoRow"><span>Current RaceHub Space</span><b>${safe(s.name)}</b></div><div class="rhInfoRow"><span>Driver Profile (Global)</span><b>${safe(state.driverName||'Driver')}</b></div></section>
