...
window.rhDataBackups=function(){
 const s=rhSpace(),bs=(s.backups||[]).slice().reverse();show('more');q('more').innerHTML=`<div class="rhScene rhSettingsScene">${rhHeader('DATA / BACKUPS','Protect your RaceHub data','settings','more')}</div><div class="rhContent rhConformance"><div class="rhDataActions"><button class="btn" onclick="rhBackupFinal()">CREATE BACKUP<small>Save current data</small></button></div><h2 class="rhSectionTitle">SAVED BACKUPS</h2>${bs.length?bs.map((b,i)=>`<section class="rhBackupCard"><div><small>${i===0?'MOST RECENT':''}</small><h2>${safe(b.spaceName)}</h2><p>${new Date(b.date).toLocaleString('en-GB')}</p><div class="rhBackupCounts"><span>${b.counts.cars}<small>Cars</small></span><span>${b.counts.championships}<small>Championships</small></span><span>${b.counts.results}<small>Results</small></span></div></div><button class="btn" onclick="rhRestoreDetail('${b.id}')">RESTORE</button></section>`).join(''):rhEmpty('NO SAVED BACKUPS','Create a backup to protect your RaceHub data.','CREATE BACKUP','rhBackupFinal()')}</div>`;
};
...
window.rhManageSpaces=function(){
 const s=rhSpace();show('more');q('more').innerHTML=`<div class="rhScene rhSpaceScene">${rhHeader('RACEHUB SPACES','Manage your racing spaces','settings','more')}</div><div class="rhContent rhConformance">
 ...
 <button class="btn rhPrimaryWide" onclick="rhCreateSpaceFinal()">CREATE NEW SPACE</button><p class="small">Play more than one racing game? Create a separate RaceHub for each one. Each RaceHub keeps its own Garage, Championships, Records, Hall of Fame and Stats separate.</p></div>`;
};
