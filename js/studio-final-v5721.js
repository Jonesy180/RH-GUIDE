...
function rhRenderFestival(){
 ...
 $('festival').innerHTML=`<div class="rhFestivalV1">
   ...
   <main class="rhFestivalBodyV1">
     ${active?`<button class="rhFestivalContinueV1" onclick="rhOpenRun('${active.id}')">
       <i aria-hidden="true">🏁</i>
       <span><b>CONTINUE RACING</b><small>${esc(active.name)}</small></span>
       <strong>${progress.done} / ${progress.total}</strong><em aria-hidden="true">›</em>
     </button>`:''}
     ...
     <section class="rhFestivalSectionV1 rhFestivalFavouriteV1">
       ...
       ${fav?rhChampCard('favourite',fav,`${fav} Championship`,rhEligible('favourite',fav).length):`
         <div class="rhFavouriteUnset">
           <p>Choose a Favourite Manufacturer in Settings to unlock its Championship.</p>
           <button class="btn secondary" onclick="rhMoreMode='settings';show('more')">SET FAVOURITE MANUFACTURER</button>
         </div>`}
     </section>
     ...
     <div class="rhFestivalInfoV1">
       ...
       <p>Championships are generated from the cars in your Garage.<br>Only Eras and Manufacturers you have cars for are shown.<br>Use Expand to view and start available Championships.</p>
     </div>
   </main>
 </div>`;
}
...
function rhEventCompleteHtml(e){const p=rhEventProgress(e),board=rhEventLeaderboard(e);return `<div class="rhContent"><section class="rhSection rhEventCompleteHero"><div class="rhEventStatus">EVENT COMPLETE</div><h2>${esc(e.name)}</h2><p>${p.done} / ${p.total} results recorded</p></section><section class="rhSection"><h2>Final Leaderboard</h2>${board.map((x,i)=>`<div class="rhLeaderRow"><b>${i+1}</b><span>${esc(carName(x.car))}</span><strong>${rhFmtTime(x.total)}</strong></div>`).join('')}</section><section class="rhSection"><h2>Results</h2>${(e.results||[]).map(x=>{const car=rhSpace().cars.find(c=>c.id===x.carId);return `<div class="rhListRow"><span><b>${esc(x.roundName)}</b><small>${car?esc(carName(car)):'Unknown car'}</small></span><strong>${rhFmtTime(x.time)}</strong></div>`}).join('')}</section><button class="btn rhReturnEvents" onclick="rhRenderEvents();show('events')">RETURN TO EVENTS</button></div>`}
...
function rhRenderGarage(){
 ...
 const grouped={};
 s.cars.forEach(c=>{if(q&&!carName(c).toLowerCase().includes(q))return;(grouped[c.make]||(grouped[c.make]=[])).push(c)});
 const makes=Object.keys(grouped).sort((a,b)=>a.localeCompare(b));
 $('garage').innerHTML=`<div class="rhScene rhGarageScene">${rhHeader('GARAGE','Your Cars','garage')}</div><div class="rhContent"><section class="rhSection"><div class="rhGarageCount"><b>${s.cars.length}</b><span>cars in ${esc(s.name)}</span></div><div class="rhGarageTools"><input id="garageSearch" placeholder="Search make, model or year" value="${esc(garageSearch)}" oninput="garageSearch=this.value;rhRenderGarage()"><button class="chip" onclick="rhOpenCarEditor()">＋ Add Car</button></div>${makes.length?`<div class="rhMakeAccordion">${makes.map(make=>{const cars=grouped[make].slice().sort((a,b)=>carName(a).localeCompare(carName(b)));const open=q||rhGarageOpenMake===make;return `<section class="rhMakeGroup ${open?'open':''}"><button class="rhMakeHead" onclick="rhToggleGarageMake('${esc(make).replace(/'/g,'&#39;')}')"><span><b>${esc(make)}</b><small>${cars.length} car${cars.length===1?'':'s'}</small></span><em>${open?'⌄':'›'}</em></button>${open?`<div class="rhMakeCars">${cars.map(c=>`<div class="rhCar"><span><strong>${esc(c.model)}</strong><small>${esc(c.year||'')}</small></span><button class="rhEditIcon" aria-label="Edit ${esc(carName(c))}" onclick="rhOpenCarEditor('${c.id}')">✎</button></div>`).join('')}</div>`:''}</section>`}).join('')}</div>`:rhEmpty(q?'NO CARS FOUND':'YOUR GARAGE IS EMPTY',q?'Try a different make, model or year.':'Add cars to start building Championships and recording your racing.',q?'Clear Search':'Add Your First Car',q?"garageSearch='';rhRenderGarage()":'rhOpenCarEditor()')}</section></div>`}
function rhToggleGarageMake(make){rhGarageOpenMake=rhGarageOpenMake===make?null:make;rhRenderGarage()}
...
function rhRecordsHeader(hall){
 if(!hall)return rhHeader('RECORDS','Your Racing History','records');
 return `<div class="rhPageHead"><button class="rhBack" onclick="rhRecordsMode='records';rhRenderRecords()">‹</button><div><h1>HALL OF FAME</h1><p>Completed Championship winners</p></div><button class="rhHelp" onclick="rhOpenHelp('records')">?</button></div>`
}
function rhRecordRoundRow(r,rd){
 ...
 return `<div class="rhRecordRow"><div class="rhRecordIdentity"><b>${esc(rd.name)}</b><small>${best&&car?esc(carName(car)):'No result recorded'}</small></div><strong>${best?rhFmtTime(best.time):'—'}</strong></div>`
}
function rhRenderRecords(){const runs=rhCurrentRuns(),completed=runs.filter(r=>r.status==='complete'),hall=rhRecordsMode==='hall';$('hall').innerHTML=`${rhRecordsHeader(hall)}<div class="rhContent">${hall?rhHallOfFame(completed):`<button class="rhHallBanner" onclick="rhRecordsMode='hall';rhRenderRecords()"><span class="rhHallBannerIcon">🏆</span><span class="rhHallBannerCopy"><b>HALL OF FAME</b><small>Completed Championship winners</small></span><em>›</em></button><section class="rhSection"><h2>Championship Records</h2>${runs.length?runs.slice().reverse().map(r=>`<details class="rhRecordRun"><summary><span><b>${esc(r.name)}</b><small>${r.status==='complete'?'Completed':'Active'} • ${r.results.length} result${r.results.length===1?'':'s'}</small></span></summary><div class="rhRecordRows">${r.rounds.map(rd=>rhRecordRoundRow(r,rd)).join('')}</div></details>`).join(''):rhEmpty('NO RECORDS YET','Championship records will appear here after you start recording results.','View Championships',"show('festival')")}</section>`}</div>`}
