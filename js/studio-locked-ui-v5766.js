...
window.rhEventResultSummary=function(e,res){
 ...
 show('event');resultScreen(q('event'),'RESULTS SUMMARY',res.roundName,`<section class="rhResultsSummary rhResultsSummaryFinal">
  ...
  ${rhAverageCompareHtml(hist)}${records?`<div class="rhRecordCelebration rhRecordCelebrationFinal"><div class="rhRosettes">${res.championshipRecord?'<b>✹ EVENT RECORD</b>':''}${res.allTime?'<b class="gold">✹ ALL-TIME RACEHUB RECORD</b>':''}</div><img src="assets/final/hubs.png" alt="Hubs"><p>Incredible driving,<br><b>well done!</b></p></div>`:''}
...
window.rhShowCompletedEventLeaderboard=function(eventId){
 ...
 q('event').innerHTML=`<div class="rhFinalBoardV1 rhFinalEventBoardV1"><main class="rhFinalBoardBodyV1">
  ...
  <section class="rhFinalCompleteNoteV1"><b>EVENT COMPLETE</b><span>${board.length} racers classified • Final classification saved to RaceHub.</span></section>
...
window.rhDataBackups=function(){
 const s=rhSpace(),bs=(s.backups||[]).slice().reverse();show('more');q('more').innerHTML=`<div class="rhScene rhSettingsScene">${rhHeader('DATA / BACKUPS','Protect your RaceHub data','settings','more')}</div><div class="rhContent rhConformance rhBackupsFinal">
 <div class="rhDataActions rhDataActionsFinal"><button class="btn" onclick="rhBackupFinal()"><b>CREATE BACKUP</b><small>Save current data</small></button></div>
 <h2 class="rhSectionTitle rhBackupHeadingFinal">SAVED BACKUPS</h2>
 ${bs.length?bs.map((b,i)=>`<section class="rhBackupCard rhBackupCardFinal">
  <div class="rhBackupTopFinal"><div><small>${i===0?'MOST RECENT':'SAVED BACKUP'}</small><h2>${safe(b.spaceName)}</h2><p>${new Date(b.date).toLocaleString('en-GB')}</p></div></div>
  <div class="rhBackupCounts rhBackupCountsFinal"><span><b>${b.counts.cars}</b><small>Cars</small></span><span><b>${b.counts.championships}</b><small>Championships</small></span><span><b>${b.counts.results}</b><small>Results</small></span></div>
  <button class="btn rhBackupRestoreFinal" onclick="rhRestoreDetail('${b.id}')">RESTORE</button>
 </section>`).join(''):rhEmpty('NO SAVED BACKUPS','Create a backup to protect your RaceHub data.','CREATE BACKUP','rhBackupFinal()')}</div>`;
};
...
window.rhRestoreDetail=function(id){const s=rhSpace(),b=(s.backups||[]).find(x=>x.id===id);if(!b)return;show('more');q('more').innerHTML=`<div class="rhScene rhSettingsScene">${rhHeader('RESTORE BACKUP','Current Space only','settings','more')}</div><div class="rhContent rhConformance"><section class="rhRestoreCard"><h2>${safe(b.spaceName)}</h2><p>${new Date(b.date).toLocaleString('en-GB')}</p><div class="rhBackupCounts"><span>${b.counts.cars}<small>Cars</small></span><span>${b.counts.championships}<small>Championships</small></span><span>${b.counts.results}<small>Results</small></span></div></section><section class="rhWarning"><h2>IMPORTANT</h2><p>Restoring this backup will replace the current data in <b>${safe(s.name)}</b>.</p><b>This cannot be undone.</b><p>Your other RaceHub Spaces are not affected.</p></section><div class="rhModalActions"><button class="btn secondary" onclick="rhDataBackups()">CANCEL</button><button class="btn dangerBtn" onclick="rhRestoreConfirmFinal('${id}')">RESTORE BACKUP</button></div></div>`};
window.rhRestoreConfirmFinal=function(id){const b=(rhSpace().backups||[]).find(x=>x.id===id);if(!b)return;rhConfirm({title:'RESTORE THIS BACKUP?',copy:'This will replace all data in the current RaceHub Space with the selected backup.',detail:`${b.spaceName} • ${b.counts.cars} Cars • ${b.counts.championships} Championships • ${b.counts.results} Results`,safeguard:'Your other RaceHub Spaces will not be affected.',confirmLabel:'RESTORE',severity:'cyan',onConfirm:`rhRestoreFinal('${id}')`})};
...
window.rhAbout=function(){
 ...
 <section class="rhSection"><h2>RACEHUB INFORMATION</h2><div class="rhInfoRow"><span>App Version</span><b>v5.7.67</b></div><div class="rhInfoRow"><span>Data / Backup Version</span><b>v1.0.0</b></div><div class="rhInfoRow"><span>Current RaceHub Space</span><b>${safe(s.name)}</b></div><div class="rhInfoRow"><span>Driver Profile (Global)</span><b>${safe(state.driverName||'Driver')}</b></div></section>
...
window.rhManageSpaces=function(){
 const s=rhSpace();show('more');q('more').innerHTML=`<div class="rhScene rhSpaceScene">${rhHeader('RACEHUB SPACES','Manage your racing spaces','settings','more')}</div><div class="rhContent rhConformance rhSpacesFinal">
 ...
 <p class="small rhSpacesHelpFinal">Play more than one racing game? Create a separate RaceHub for each one. Each RaceHub keeps its own Garage, Championships, Records, Hall of Fame and Stats separate.</p></div>`;
};
window.rhSwitchSpace=function(id){if(!state.spaces.some(x=>x.id===id))return;state.activeSpaceId=id;rhSync();rhSave();toast('RaceHub Space switched');rhManageSpaces()};

window.rhCreateSpace=function(){rhCreateSpaceFinal()};
window.rhCreateSpaceFinal=function(){q('rhSpaceCreate')?.remove();document.body.insertAdjacentHTML('beforeend',`<div id="rhSpaceCreate" class="rhOverlay"><div class="rhModal rhFormModal"><button class="rhModalX" onclick="$('rhSpaceCreate').remove()">×</button><h2>CREATE RACEHUB SPACE</h2><label>RaceHub Name</label><input id="rhNewSpaceName" placeholder="Enter game name"><div class="rhModalActions"><button class="btn secondary" onclick="$('rhSpaceCreate').remove()">CANCEL</button><button class="btn" onclick="rhCreateSpaceSave()">CREATE SPACE</button></div></div></div>`)};
window.rhCreateSpaceSave=function(){const name=q('rhNewSpaceName')?.value.trim();if(!name)return toast('Enter a RaceHub name');const s=rhSpaceTemplate(name,[]);state.spaces.push(s);state.activeSpaceId=s.id;rhSync();rhSave();q('rhSpaceCreate')?.remove();rhManageSpaces()};
window.rhRenameSpacePrompt=function(id){const x=state.spaces.find(s=>s.id===id);if(!x)return;q('rhSpaceRename')?.remove();document.body.insertAdjacentHTML('beforeend',`<div id="rhSpaceRename" class="rhOverlay"><div class="rhModal rhFormModal"><h2>RENAME RACEHUB SPACE</h2><input id="rhRenameSpaceName" value="${safe(x.name)}"><div class="rhModalActions"><button class="btn secondary" onclick="$('rhSpaceRename').remove()">CANCEL</button><button class="btn" onclick="rhRenameSpaceSave('${id}')">SAVE</button></div></div></div>`)};
...
window.rhChangeFavourite=function(value){const s=rhSpace(),old=s.favouriteManufacturer;if(value===old)return;if(!old&&value){rhConfirm({title:'SET FAVOURITE MANUFACTURER?',copy:'Set your Favourite Manufacturer for this RaceHub Space.',detail:`Not set → ${value}`,safeguard:'Your Garage and other Championships will not be affected.',confirmLabel:'SET FAVOURITE',severity:'purple',onConfirm:`rhChangeFavouriteFinal(${JSON.stringify(value)})`});}else{rhConfirm({title:'CHANGE FAVOURITE MANUFACTURER?',copy:'Changing your Favourite Manufacturer will delete the current Favourite Manufacturer Championship and its progress.',detail:`${old||'Not set'} → ${value||'Not set'}`,safeguard:'Your other Championships and Garage will not be affected.',confirmLabel:'CHANGE FAVOURITE',severity:'purple',onConfirm:`rhChangeFavouriteFinal(${JSON.stringify(value)})`});}rhRenderSettings()};
...
window.rhRecordBadges=function(best,label='CHAMPIONSHIP'){if(!best)return '';return `<span class="rhRecordBadges">${best.championshipRecord?`<em>${label} RECORD</em>`:''}${best.allTime?'<em class="gold">ALL-TIME RACEHUB RECORD</em>':''}</span>`};
...
window.rhOnboardingStep=function(step=1){
 ...
 const makes=[...new Set((Array.isArray(SEED?.cars)?SEED.cars:[]).map(c=>c.make).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
 let body='';
 ...
 else if(step===2)body=`<small>STEP 2 OF 3</small><h1>FAVOURITE MANUFACTURER</h1><p>Choose a Favourite Manufacturer for this RaceHub Space.</p><input id="rhOnMakeSearch" placeholder="Search manufacturers" oninput="rhOnboardingFilterMakes(this.value)"><div id="rhOnMakeList" class="rhOnMakeList">${makes.map(m=>`<button data-make="${safe(m).toLowerCase()}" onclick="rhOnboardingChooseMake(${JSON.stringify(m)})">${safe(m)}</button>`).join('')}</div><button class="btn secondary" onclick="rhOnboardingStep(1)">BACK</button>`;
...
window.rhOnboardingFilterMakes=function(v){const x=String(v||'').toLowerCase();q('rhOnMakeList')?.querySelectorAll('button').forEach(b=>b.hidden=!b.dataset.make.includes(x))};
window.rhOnboardingChooseMake=function(m){rhSpace().favouriteManufacturer=m;rhSave();rhOnboardingStep(3)};
