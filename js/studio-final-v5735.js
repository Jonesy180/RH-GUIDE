...
function rhOpenRun(id){const r=rhCurrentRuns().find(x=>x.id===id);if(!r)return;show('festival');const p=rhRunProgress(r),next=rhNextSlot(r);if(r.status==='complete'){$('festival').innerHTML=`<div class="rhScene rhChampScene">${rhHeader('CHAMPIONSHIP',r.name,'festival','festival')}</div><div class="rhContent"><section class="rhRunOverview"><img src="${rhTrophy(r.trophy)}"><div><span>CHAMPIONSHIP COMPLETE</span><h2>${esc(r.name)}</h2><p>Created ${new Date(r.createdAt).toLocaleDateString('en-GB')}</p></div></section><section class="rhSection"><div class="rhProgressTop"><b>Progress</b><span>${p.done} / ${p.total}</span></div><div class="progress"><div class="bar" style="width:${p.pct}%"></div></div></section><section class="rhSection"><h2>Frozen Entry List</h2>${r.entries.map(id=>`<div class="rhListRow">${esc(carName(carById(id)||{make:'Unknown',model:'Car'}))}</div>`).join('')}</section><section class="rhSection"><h2>Rounds</h2>${r.rounds.map((x,i)=>`<div class="rhListRow"><b>${i+1}</b><span>${esc(x.name)}</span></div>`).join('')}</section>${rhLeaderboard(r)}</div>`;return}const currentCar=next?carById(next.carId):null,currentIndex=next?r.entries.indexOf(next.carId):-1,currentDone=next?r.results.filter(x=>x.carId===next.carId).length:0;const typeLabel=rhSetupTypeLabel(r.type||'festival');$('festival').innerHTML=`<div class="rhOverviewV1">
 ...
 <main class="rhOverviewBodyV1">
  <section class="rhOverviewProgressV1">
   ...
   <div class="rhOverviewStatsV1"><div><b>${r.entries.length}</b><span>FROZEN CARS</span></div><div><b>${r.rounds.length}</b><span>ROUNDS</span></div><div><b>${p.done}</b><span>RESULTS SAVED</span></div></div>
  </section>
  ${next?`<section class="rhCurrentCarV1">
   <div class="rhCurrentCarHeadV1"><div><small>CURRENT CAR</small><h2>${esc(carName(currentCar||{make:'Unknown',model:'Car'}))}</h2></div><strong>${currentIndex+1} OF ${r.entries.length}</strong></div>
   <div class="rhCurrentCarMetaV1"><div><span>CURRENT ROUND</span><b>${esc(next.round.name)}</b></div><div><span>CAR PROGRESS</span><b>${currentDone} OF ${r.rounds.length} ROUNDS COMPLETE</b></div></div>
  ...
  <section class="rhLineupV1">
   ...
   <div class="rhLineupRowsV1">${r.entries.map((cid,i)=>{const c=carById(cid),done=r.results.filter(x=>x.carId===cid).length,isCurrent=next&&cid===next.carId,isComplete=done>=r.rounds.length;return `<div class="rhLineupRowV1 ${isCurrent?'current':''} ${isComplete?'complete':''}"><i>${i+1}</i><div><b>${esc(carName(c||{make:'Unknown',model:'Car'}))}</b><small>${done} / ${r.rounds.length} rounds complete</small></div><strong>${isComplete?'COMPLETE':isCurrent?'CURRENT':'WAITING'}</strong></div>`}).join('')}</div>
  ...
  <section class="rhFrozenV1">
   ...
   <div class="rhFrozenGridV1"><div><b>ENTRY LIST</b>${r.entries.map((cid,i)=>`<p><span>${i+1}</span>${esc(carName(carById(cid)||{make:'Unknown',model:'Car'}))}</p>`).join('')}</div><div><b>ROUNDS</b>${r.rounds.map((rd,i)=>`<p><span>${i+1}</span>${esc(rd.name)}</p>`).join('')}</div></div>
