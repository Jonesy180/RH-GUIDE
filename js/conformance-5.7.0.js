...
/* RaceHub v5.7.0 — Studio Final Conformance Candidate
...
window.rhResultSummary=function(r,res){
 ...
 ${records?`<div class="rhRecordCelebration"><div class="rhRosettes">${res.championshipRecord?'<b>✹ CHAMPIONSHIP RECORD</b>':''}${res.allTime?'<b class="gold">✹ ALL-TIME RACEHUB RECORD</b>':''}</div><img src="assets/final/hubs.png" alt="Hubs"><p>Incredible driving,<br><b>well done!</b></p></div>`:''}
...
window.rhEventResultSummary=function(e,res){
 ...
 ${records?`<div class="rhRecordCelebration"><div class="rhRosettes">${res.championshipRecord?'<b>✹ EVENT RECORD</b>':''}${res.allTime?'<b class="gold">✹ ALL-TIME RACEHUB RECORD</b>':''}</div><img src="assets/final/hubs.png"><p>Incredible driving,<br><b>well done!</b></p></div>`:''}
...
window.rhAbout=function(){const s=rhSpace();show('more');q('more').innerHTML=`<div class="rhScene rhSettingsScene">${rhHeader('ABOUT RACEHUB','DRIVE • RECORD • IMPROVE','settings','more')}</div><div class="rhContent rhConformance"><section class="rhAboutIntro"><h1>ABOUT RACEHUB</h1><h3>DRIVE • RECORD • IMPROVE</h3><p>RaceHub is your personal racing record book — built to organise your Garage, create Championships, record results and preserve your racing history your way.</p><p>Whether you race for fun, for competition, or just for the love of driving, RaceHub keeps your racing history safe.</p></section><section class="rhSection"><h2>RACEHUB INFORMATION</h2><div class="rhInfoRow"><span>App Version</span><b>v5.7.0</b></div><div class="rhInfoRow"><span>Data / Backup Version</span><b>v1.0.0</b></div><div class="rhInfoRow"><span>Current RaceHub Space</span><b>${safe(s.name)}</b></div><div class="rhInfoRow"><span>Driver Profile (Global)</span><b>${safe(state.driverName||'Driver')}</b></div></section><section class="rhSection"><h2>CREATED BY</h2><p>Andy Jones & ChatGPT</p><p class="small">Designed together from the ground up to make racing data personal, useful and enjoyable.</p></section><section class="rhSection"><h2>LEGAL</h2><p>© 2026 RaceHub. All rights reserved.</p><p class="small">RaceHub is an independent product and is not affiliated with or endorsed by any vehicle manufacturer.</p></section></div>`};

window.rhManageSpaces=function(){const s=rhSpace();show('more');q('more').innerHTML=`<div class="rhScene rhSettingsScene">${rhHeader('RACEHUB SPACES','Manage your racing spaces','settings','more')}</div><div class="rhContent rhConformance">${state.spaces.map(x=>`<section class="rhSpaceManage ${x.id===s.id?'active':''}"><div><b>${safe(x.name)}</b><small>${x.cars.length} cars • ${(x.runs||[]).length} Championships ${x.id===s.id?'• CURRENT':''}</small></div><div><button class="btn secondary" onclick="rhRenameSpacePrompt('${x.id}')">RENAME</button>${state.spaces.length>1?`<button class="btn dangerBtn" onclick="rhDeleteSpaceConfirm('${x.id}')">DELETE</button>`:''}</div></section>`).join('')}<button class="btn rhPrimaryWide" onclick="rhCreateSpaceFinal()">CREATE NEW SPACE</button><p class="small">Play more than one racing game? Create a separate RaceHub for each one. Each RaceHub keeps its own Garage, Championships, Records, Hall of Fame and Stats separate.</p></div>`};
...
<section class="rhSection rhSettingPanel"><button class="rhSettingRow" onclick="rhAbout()"><b>ABOUT RACEHUB</b><span>RaceHub v5.7.0 • ›</span></button></section></div>`};
