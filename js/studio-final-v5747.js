...
function rhShuffleQueue(id){const r=rhCurrentRuns().find(x=>x.id===id);if(!r)return;const remaining=rhQueueRemaining(r);if(remaining.length<2){toast('Not enough remaining cars to shuffle');return}for(let i=remaining.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[remaining[i],remaining[j]]=[remaining[j],remaining[i]]}r.entries=[...rhQueueCompleted(r),...remaining];rhSave();toast('Queue shuffled');rhOpenRun(id)}
function rhRandomPickQueue(id){const r=rhCurrentRuns().find(x=>x.id===id);if(!r)return;const remaining=rhQueueRemaining(r);if(!remaining.length){toast('No remaining cars');return}const pick=remaining[Math.floor(Math.random()*remaining.length)],completed=rhQueueCompleted(r),rest=remaining.filter(cid=>cid!==pick);r.entries=[...completed,pick,...rest];rhSave();toast(`Random pick: ${carName(carById(pick)||{make:'Unknown',model:'Car'})}`);rhOpenRun(id)}
function rhOpenRun(id){const r=rhCurrentRuns().find(x=>x.id===id);if(!r)return;show('festival');const p=rhRunProgress(r),next=rhNextSlot(r);if(r.status==='complete'){const rows=r.entries.map(id=>{const rr=r.results.filter(x=>x.carId===id);return rr.length===r.rounds.length?{id,total:rr.reduce((a,b)=>a+b.time,0)}:null}).filter(Boolean).sort((a,b)=>a.total-b.total),winner=rows[0],typeLabel=rhSetupTypeLabel(r.type||r.championshipType||'festival');$('festival').innerHTML=`<div class="rhFinalBoardV1">
 ...
 <main class="rhOverviewBodyV1">
  ...
  <section class="rhLineupV1 rhQueuePanelV1">
   ...
   <p class="rhQueueNoteV1">The Championship entry list is frozen. Random Pick and Shuffle only change the order of cars that still have racing to complete.</p>
  </section>
