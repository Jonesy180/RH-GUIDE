...
window.rhOnboardingStep=function(step=1){
 ...
 else if(step===2)body=`<small>STEP 2 OF 3</small><h1>FAVOURITE MANUFACTURER</h1><p>Choose a Favourite Manufacturer for this RaceHub Space.</p><input id="rhOnMakeSearch" placeholder="Search manufacturers" oninput="rhOnboardingFilterMakes(this.value)"><div id="rhOnMakeList" class="rhOnMakeList">${makes.map(m=>`<button data-make="${safe(m).toLowerCase()}" onclick="rhOnboardingSelectMake(${JSON.stringify(m)})">${safe(m)}</button>`).join('')}</div><button id="rhOnMakeContinue" class="btn rhPrimaryWide" disabled onclick="rhOnboardingConfirmMake()">SET FAVOURITE &amp; CONTINUE</button><button class="btn secondary" onclick="rhOnboardingStep(1)">BACK</button>`;
