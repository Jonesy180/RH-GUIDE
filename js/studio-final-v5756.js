...
function rhOpenEvent(id){const e=rhSpace().customEvents.find(x=>x.id===id);if(!e)return;e.racerCount=rhEventRacerCount(e);e.carIds=Array.isArray(e.carIds)?e.carIds.filter(id=>rhSpace().cars.some(c=>c.id===id)).slice(0,e.racerCount):[];if(e.status!=='complete'&&!e.startedAt&&(e.results||[]).length)e.startedAt=e.results[0]?.date||new Date().toISOString();show('event');$('event').innerHTML=`<div class="rhScene rhEventsScene">${rhHeader(e.status==='complete'?'EVENT COMPLETE':'EVENT',e.name,'events','events')}</div>${e.status==='complete'?rhEventCompleteHtml(e):rhEventIsStarted(e)?rhEventActiveHtml(e):rhEventSetupHtml(e)}`}
