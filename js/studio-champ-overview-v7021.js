...
(function(){
  ...
  function competitionLabel(r){
    if(r && (r.pickMyDrive||r.value==='pick-my-drive')) return 'PICK MY DRIVE';
  ...
  window.rhOpenRun=function(id){
    ...
    $('festival').innerHTML=`<div class="rhChamp33">
      ...
      <main class="rhChamp33Body">
        ...
        ${(r.pickMyDrive||r.value==='pick-my-drive')?'':`<section class="rhChamp33Tools">
        ...
        <footer class="rhChamp33Footer">
          ...
          <button onclick="${(r.pickMyDrive||r.value==='pick-my-drive')?'rhPickMyDriveComingSoon()':'rhRenderFestival()'}"><i>←</i><b>BACK TO<br>${(r.pickMyDrive||r.value==='pick-my-drive')?'PICK MY DRIVE':'FESTIVAL'}</b></button>
        </footer>
      </main>
    </div>`;
  };
