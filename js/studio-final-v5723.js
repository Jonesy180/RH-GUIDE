...
function rhRenderGarage(){
 ...
 const q=garageSearch.trim().toLowerCase(),grouped={};
 ...
 $('garage').innerHTML=`<div class="rhGarageV1">
  ...
  <main class="rhGarageBodyV1">
   ...
   <div class="rhGarageToolsV1"><label><i>⌕</i><input id="garageSearch" placeholder="Search your cars..." value="${esc(garageSearch)}" oninput="garageSearch=this.value;rhRenderGarage()"></label><button onclick="rhOpenCarEditor()">⊕ <span>ADD NEW CAR</span></button></div>
   ${makes.length?`<div class="rhGarageMakesV1">${makes.map(make=>{const cars=grouped[make].slice().sort((a,b)=>carName(a).localeCompare(carName(b)));const open=!!q||rhGarageOpenMake===make;return `<section class="rhGarageMakeV1 ${open?'open':''}"><button class="rhGarageMakeHeadV1" onclick="rhToggleGarageMake('${esc(make).replace(/'/g,'&#39;')}')"><b>${esc(make)}</b><span>${cars.length} Car${cars.length===1?'':'s'}</span><em>${open?'⌃':'⌄'}</em></button>${open?`<div class="rhGarageCarsV1">${cars.map(c=>`<div class="rhGarageCarV1"><i>•</i><span><b>${esc(c.model)}</b><small>${esc(c.year||'')}</small></span><button aria-label="Edit ${esc(carName(c))}" onclick="event.stopPropagation();rhOpenCarEditor('${c.id}')">✎</button></div>`).join('')}</div>`:''}</section>`}).join('')}</div>`:rhEmpty(q?'NO CARS FOUND':'YOUR GARAGE IS EMPTY',q?'Try a different make, model or year.':'Add cars to start building Championships and recording your racing.',q?'Clear Search':'Add Your First Car',q?"garageSearch='';rhRenderGarage()":'rhOpenCarEditor()')}
