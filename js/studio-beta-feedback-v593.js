/* RaceHub / rh-guide v5.9.3 — synchronized beta feedback fixes. */
(()=>{
  'use strict';
...
/* v5.9.3 — authoritative About version + all back-arrow variants. */
(()=>{
  'use strict';
  const VERSION=document.querySelector('meta[name="racehub-version"]')?.content||'5.9.3';
  ...
  function repairAboutVersion(){
    document.querySelectorAll('.rhSettingRow').forEach(row=>{
      ...
      if(value)value.textContent=`rh-guide v${VERSION} • ›`;
    });
  }

  function repairBackArrows(root=document){
    root.querySelectorAll('button').forEach(button=>{
      const raw=(button.textContent||'').trim();
      const isKnownClass=button.matches('.rhBack,.rhPostRaceBack,.rhResultBack,.rhRecordsHeadV1>button,.rhStandings34Header>button,.rhStandings33 header button');
      const isArrowOnly=/^[‹〈❮←]$/.test(raw);
      if(!isKnownClass&&!isArrowOnly)return;
      button.classList.add('rhBackCenteredV593');
      let glyph=button.querySelector(':scope > .rhBackGlyphV593');
      if(!glyph){
        button.textContent='';
        glyph=document.createElement('span');
        glyph.className='rhBackGlyphV593';
        glyph.textContent='‹';
  ...
  if(typeof settings==='function')window.rhRenderSettings=function(){
    ...
    repairAboutVersion();
    repairBackArrows(document.getElementById('more')||document);
    return out;
  };
  ...
  const observer=new MutationObserver(records=>{
    for(const record of records){
      for(const node of record.addedNodes){
        ...
        repairAboutVersion();
        repairBackArrows(node.matches?.('button')?node.parentElement||document:node);
      }
    }
  });
  observer.observe(document.body,{childList:true,subtree:true});
  setTimeout(()=>{repairAboutVersion();repairBackArrows();},0);
})();
