...
function rhLoad(){
 try{
  const x=JSON.parse(localStorage.getItem(RH_FINAL_STORE)||'null');
  if(x?.schema===2&&Array.isArray(x.spaces)&&x.spaces.length){
   const valid=id=>x.spaces.some(s=>s.id===id);
   if(!valid(x.defaultSpaceId))x.defaultSpaceId=valid(x.activeSpaceId)?x.activeSpaceId:x.spaces[0].id;
   x.activeSpaceId=x.defaultSpaceId;
   localStorage.setItem(RH_FINAL_STORE,JSON.stringify(x));
   return x
  }
 }catch(e){}
 try{const old=JSON.parse(localStorage.getItem(STORE)||'null');if(old?.cars){const x=rhMigrateLegacy(old);x.defaultSpaceId=x.activeSpaceId;localStorage.setItem(RH_FINAL_STORE,JSON.stringify(x));return x;}}catch(e){}
...
let rhAndroidBackHandling=false;
function rhShow(screen){
 if(!rhAndroidBackHandling&&screen!=='home'){try{history.pushState({racehub:true,screen},'',location.href)}catch(e){}}
 currentScreen=screen;rhSync();document.body.dataset.screen=screen;document.querySelectorAll('.screen').forEach(s=>s.classList.add('hidden'));$(screen).classList.remove('hidden');document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.screen===screen));rhRender(screen);window.scrollTo(0,0)
}
function show(screen){rhShow(screen)}
function rhCloseTopOverlayForBack(){
 const overlays=[...document.querySelectorAll('.rhOverlay,.rhMilestone')].filter(x=>x&&x.offsetParent!==null);
 if(!overlays.length)return false;overlays[overlays.length-1].remove();return true
}
window.addEventListener('popstate',()=>{
 if(rhCloseTopOverlayForBack()){if(currentScreen&&currentScreen!=='home'){try{history.pushState({racehub:true,screen:currentScreen},'',location.href)}catch(e){}}return}
 if(!currentScreen||currentScreen==='home')return;
 const host=document.getElementById(currentScreen),back=host?.querySelector('.rhBack:not([disabled])');
 rhAndroidBackHandling=true;
...
function rhRenderHome(){
 const s=rhSpace();
 const tile=(cls,onclick,icon,title,sub,wide=false)=>`<button class="v7DashTile ${cls}${wide?' wide':''}" onclick="${onclick}"><i aria-hidden="true">${icon}</i><span><b>${title}</b><small>${sub}</small></span><em aria-hidden="true">›</em></button>`;
 const icons={
 ...
 $('home').innerHTML=`<div class="v7Dash">
   <section class="v7DashHero">
     <div class="v7DashWelcome"><small>WELCOME TO</small><b>${esc(s.name)}</b></div>
     <img class="v7DashLogo" src="assets/brand/otg-approved-painted-mark-v6052.png" alt="OTG! — Out The Garage!">
   </section>
   <main class="v7DashBody"><div class="v7DashGrid">
     ${tile('festival',"show('festival')",'🏁','FESTIVAL','OTG! Championships',true)}
     ${tile('raceoff',"show('raceoff')",icons.raceoff,'RACE OFF','Knockout Racing')}
     ${tile('custom',"show('events')",icons.custom,'CUSTOM RACING','Create Your Own')}
     ${tile('garage',"show('garage')",icons.garage,'GARAGE','Your Cars')}
     ${tile('setups','rhOpenRaceSetupsV7()',icons.setups,'RACE SETUPS','Saved Configs')}
     ${tile('pick','rhPickMyDriveComingSoon()',icons.drive,'PICK MY DRIVE','Let OTG! Choose')}
     ${tile('records',"rhRecordsMode='records';show('hall')",icons.records,'RECORDS','Results & Hall of Fame')}
     ${tile('stats',"rhMoreMode='stats';show('more')",icons.stats,'STATS','Your Racing Overview')}
     ${tile('settings',"rhMoreMode='settings';show('more')",icons.settings,'SETTINGS','Preferences & Data')}
   </div><div class="v7DashFooter"><span></span><b>OTG!</b><span></span><small>OUT THE GARAGE!</small></div></main>
 </div>`;
}
...
function rhOpenRoundNamePicker(kind,ownerId,roundId){
 const names=rhSavedRoundNames();
 if(!names.length){toast('No saved race names yet');return}
 document.getElementById('rhRoundNamePicker')?.remove();
 document.body.insertAdjacentHTML('beforeend',`<div id="rhRoundNamePicker" class="rhOverlay" onclick="if(event.target===this)rhCloseRoundNamePicker()"><div class="rhModal rhRoundNamePickerModal"><button class="rhModalX" onclick="rhCloseRoundNamePicker()">×</button><small>SAVED RACE NAMES</small><h2>Choose a Race</h2><p>Select a previously used race name, or close this list and type a new one.</p><div class="rhRoundNamePickerList">${names.map(n=>`<button onclick="rhChooseSavedRoundName('${kind}','${ownerId||''}','${roundId}',decodeURIComponent('${encodeURIComponent(n)}'))"><span>${esc(n)}</span><em>›</em></button>`).join('')}</div><button class="btn secondary" onclick="rhCloseRoundNamePicker()">TYPE A NEW NAME</button></div></div>`)
}
function rhCloseRoundNamePicker(){document.getElementById('rhRoundNamePicker')?.remove()}
function rhChooseSavedRoundName(kind,ownerId,roundId,name){
 name=String(name||'').trim();if(!name)return;
 if(kind==='champ'){
  const r=rhSetup?.rounds?.find(x=>x.id===roundId);if(!r)return;
  r.name=name;rhCloseRoundNamePicker();rhRenderSetup();return
 }
 if(kind==='event'){
  const e=rhSpace().customEvents.find(x=>x.id===ownerId),r=e?.rounds?.find(x=>x.id===roundId);if(!r)return;
...
function rhRenderSetup(){const x=rhSetup;if(!x)return;const cars=rhEligible(x.type,x.value),included=x.entries.length,rounds=x.rounds.length,trophy=rhSetupTrophyType(x.type);$('festival').innerHTML=`<div class="rhSetupV1 ${x.type==='favourite'?'rhFavouriteSetupV6031':''}">
 ...
 <main class="rhSetupBodyV1">
  ...
  <div class="rhSetupActionsV1 rhSetupActionsThreeV1">
   <button class="rhSetupStartV1 rhSetupStartGreenV1" ${!included||!rounds?'disabled':''} onclick="rhConfirmStart()">▶ <span><b>SAVE &amp; START</b><small>FREEZE ENTRY LIST &amp; ROUNDS AND BEGIN</small></span></button>
...
function rhRefreshSetupEntryUi(){
 const included=rhSetup?.entries?.length||0,count=document.getElementById('rhIncludedCount');
...
function rhRenameRoundLayout(id,v){const r=rhSetup.rounds.find(r=>r.id===id);if(r)r.layout=String(v||'').trim()}
...
function rhOpenRun(id){const r=rhCurrentRuns().find(x=>x.id===id);if(!r)return;show('festival');const p=rhRunProgress(r),cp=rhRunCarProgress(r),next=rhNextSlot(r);if(r.status==='complete'){return window.rhShowLockedFinalLeaderboardV5788(id)}const entries=Array.isArray(r.entries)?r.entries:[],rounds=Array.isArray(r.rounds)?r.rounds:[],results=Array.isArray(r.results)?r.results:[];const currentCar=next?carById(next.carId):null,currentIndex=next?entries.indexOf(next.carId):-1,currentDone=next?results.filter(x=>x.carId===next.carId).length:0;const typeLabel=r.pickMyDrive?'PICK MY DRIVE':rhSetupTypeLabel(r.type||r.championshipType||'festival');$('festival').innerHTML=`<div class="rhOverviewV1">
 ...
 <main class="rhOverviewBodyV1">
  <section class="rhOverviewProgressV1">
   ...
   <div class="rhOverviewProgressTrackV1"><i style="width:${p.pct}%"></i></div>
...
function rhRenderEvents(){
 const s=rhSpace(),active=s.customEvents.filter(e=>!['complete','abandoned'].includes(e.status)),hasEvents=active.length;
 ...
 $('events').innerHTML=`<div class="rhEventsV1">
   ...
   <main class="rhEventsBodyV1">
     ...
     <section class="rhEventsListV1">
       <h2>YOUR EVENTS</h2>
       ${hasEvents?`<h3>ACTIVE EVENTS</h3>${active.map(rhEventCard).join('')}`:rhEmpty('NO ACTIVE EVENTS','Create an Event to build your own racing programme. Completed Events remain available in Records.','Create Event','rhCreateEvent()')}
     </section>
...
function rhEventSetupHtml(e){const chosen=rhEventChosenCars(e);return `<div class="rhContent"><section class="rhSection"><h2>${esc(e.name)}</h2><p>Your created racing programme.</p></section><section class="rhSection"><div class="rhEventSectionHead"><div><h2>Racers</h2><p class="small">${chosen.length} / ${e.racerCount} selected</p></div><button class="chip" onclick="rhChooseEventRacers('${e.id}')">CHOOSE RACERS</button></div>${chosen.length?`<div class="rhEventRacerList">${chosen.map((c,i)=>`<div class="rhListRow"><span>${i+1}. ${esc(carName(c))}</span></div>`).join('')}</div>`:'<div class="empty">Choose the cars that will race in this Event.</div>'}</section><section class="rhSection"><h2>Rounds</h2><p class="small">Name the Rounds in the order each car will race them.</p>${e.rounds.map((r,i)=>`<div class="rhEventRoundWrapV1"><div class="rhRound"><b>${i+1}</b><input value="${esc(r.name)}" onchange="rhRenameEventRound('${e.id}','${r.id}',this.value)"></div><label class="rhRoundLayoutV6116"><span>LAYOUT (OPTIONAL)</span><input value="${esc(r.layout||'')}" placeholder="e.g. Nordschleife" onchange="rhRenameEventRoundLayout('${e.id}','${r.id}',this.value)"></label><button type="button" class="rhSavedRaceButtonV1" onclick="rhOpenRoundNamePicker('event','${e.id}','${r.id}')">▾ CHOOSE SAVED RACE NAME</button></div>`).join('')}${rhRoundNameList()}<button class="btn secondary" onclick="rhAddEventRound('${e.id}')">＋ ADD ROUND</button></section><button class="btn rhStartEvent" ${chosen.length!==e.racerCount||!e.rounds.length?'disabled':''} onclick="rhStartEvent('${e.id}')">START EVENT</button><p class="small rhStartNote">Starting freezes the selected racers and Rounds for this Event.</p></div>`}
function rhEventActiveHtml(e){const p=rhEventProgress(e),next=rhEventNextPair(e),cars=rhEventCars(e),rounds=rhEventRounds(e);return `<div class="rhContent"><section class="rhSection"><div class="rhEventStatus">CURRENT EVENT</div><h2>${esc(e.name)}</h2><div class="rhEventProgressHead"><b>Progress</b><span>${p.done} / ${p.total}</span></div><div class="rhEventProgressTrack"><i style="width:${p.pct}%"></i></div></section>${next?`<section class="rhSection"><h2>Race Night</h2><div class="rhEventNext"><small>NEXT</small><b>${esc(carName(next.car))}</b><span>${esc(next.round.name)}</span></div><button class="btn" onclick="rhEventResult('${e.id}')">CONTINUE</button></section>`:''}<section class="rhSection"><h2>Frozen Racers</h2>${cars.map(c=>`<div class="rhListRow"><span>${esc(carName(c))}</span></div>`).join('')}</section><section class="rhSection"><h2>Rounds</h2>${rounds.map((r,i)=>`<div class="rhListRow"><span>${i+1} &nbsp; ${esc(r.name)}</span></div>`).join('')}</section>${e.results.length?`<section class="rhSection"><h2>Results</h2>${e.results.map(x=>{const car=rhSpace().cars.find(c=>c.id===x.carId);return `<div class="rhListRow"><span><b>${esc(x.roundName)}</b><small>${car?esc(carName(car)):'Unknown car'}</small></span><strong>${rhFmtTime(x.time)}</strong></div>`}).join('')}</section>`:''}</div>`}
function rhEventCompleteHtml(e){const p=rhEventProgress(e),board=rhEventLeaderboard(e),winner=board[0],eventCars=rhEventCars(e),completedCars=eventCars.filter(c=>rhEventCarIsComplete(e,c.id)).length;return `<div class="rhFinalBoardV1 rhFinalEventBoardV1">
...
function rhOpenEvent(id){const e=rhSpace().customEvents.find(x=>x.id===id);if(!e)return;if(e.status==='complete'){rhRecordsMode='records';show('hall');return};e.racerCount=rhEventRacerCount(e);e.carIds=Array.isArray(e.carIds)?e.carIds.filter(id=>rhSpace().cars.some(c=>c.id===id)).slice(0,e.racerCount):[];if(e.status!=='complete'&&!e.startedAt&&(e.results||[]).length)e.startedAt=e.results[0]?.date||new Date().toISOString();show('event');$('event').innerHTML=`<div class="rhScene rhEventsScene">${rhHeader(e.status==='complete'?'EVENT COMPLETE':'EVENT',e.name,'events','events')}</div>${e.status==='complete'?rhEventCompleteHtml(e):rhEventIsStarted(e)?rhEventActiveHtml(e):rhEventSetupHtml(e)}`}
...
function rhRenderGarage(){
 ...
 $('garage').innerHTML=`<div class="rhGarageV1">
  ...
  <main class="rhGarageBodyV1">
   ...
   <div class="rhGarageV6Guide"><b>MORE DETAILS = MORE CHAMPIONSHIPS</b><span>Adding Year and Class/Type gives OTG! more ways to group your cars. Missing details remain UNKNOWN.</span></div>
...
function rhOpenCarEditor(id=''){
 ...
 document.body.insertAdjacentHTML('beforeend',`<div id="rhCarEditor" class="rhOverlay"><div class="rhModal rhFormModal"><button class="rhModalX" onclick="$('rhCarEditor').remove()">×</button><h2>${c?(rhGarageNeedsDetails(c)?'Add Details':'Edit Car'):'Add Car'}</h2><p>${c?'Correct this Garage entry.':'Add a car to the current OTG! Space.'}</p><label>Manufacturer</label><div class="rhSmartSuggestWrap"><div id="rhMakeSuggestions" class="rhSmartSuggestions" hidden></div><input id="rhCarMake" class="rhSearch" autocomplete="off" autocapitalize="words" spellcheck="false" value="${esc(c?.make||'')}" placeholder="Manufacturer"></div><label>Vehicle Name</label><input id="rhCarModel" class="rhSearch" autocomplete="off" value="${esc(c?.model||'')}" placeholder="Vehicle name"><label>Year</label><input id="rhCarYear" class="rhSearch" autocomplete="off" inputmode="numeric" maxlength="4" value="${esc(c?.year||'')}" placeholder="UNKNOWN"><label>Class / Type</label><div class="rhSmartSuggestWrap"><div id="rhClassTypeSuggestions" class="rhSmartSuggestions rhSmartSuggestionsAbove" hidden></div><input id="rhCarClassType" class="rhSearch" autocomplete="off" autocapitalize="words" spellcheck="false" value="${esc(c?.classType||'')}" placeholder="UNKNOWN"></div><p class="small">Era is derived automatically from Year. Class/Type suggestions are learned only from values used in this OTG! Space.</p><div class="rhModalActions"><button class="btn secondary" onclick="$('rhCarEditor').remove()">CANCEL</button><button class="btn" onclick="rhSaveCarFinal('${id}')">${c?'SAVE CHANGES':'ADD TO GARAGE'}</button></div>${c?`<button class="btn secondary rhCarNotesButtonV6012" onclick="rhOpenCarNotesV6012('${id}')">CAR NOTES${String(c?.notes||'').trim()?' •':''}</button><button class="btn dangerBtn rhDeleteCar" onclick="rhConfirmDeleteCar('${id}')">DELETE CAR</button>`:''}</div></div>`)
 rhGarageBindClassTypeSuggestions();
 window.rhGT94BindManufacturer?.();
}
...
function rhOpenCarNotesV6012(carId){
 ...
 document.body.insertAdjacentHTML('beforeend',`<div id="rhCarNotesOverlayV6012" class="rhOverlay"><div class="rhModal rhFormModal rhCarNotesModalV6012"><button class="rhModalX" onclick="rhCloseCarNotesV6012('${c.id}')">×</button><h2>Car Notes</h2><p>${esc(c.make||'')} ${esc(c.model||'')}${c.year?` • ${esc(String(c.year))}`:''}</p><label for="rhCarNotesTextV6012">Notes</label><textarea id="rhCarNotesTextV6012" class="rhCarNotesTextV6012" maxlength="12000" placeholder="Add notes about this car...">${esc(String(c.notes||''))}</textarea><p class="small">Private notes for this car in this OTG! Space.</p><div class="rhModalActions"><button class="btn secondary" onclick="rhCloseCarNotesV6012('${c.id}')">CANCEL</button><button class="btn" onclick="rhSaveCarNotesV6012('${c.id}')">SAVE NOTES</button></div></div></div>`);
...
function rhCaptureChampEligibility(){
 const s=rhSpace(),seen=s.championshipDiscoveries||(s.championshipDiscoveries={});
 return {eligible:new Set(rhEligibleChampionshipsForCars(s.cars||[]).map(x=>rhChampDiscoveryKey(x.type,x.value))),seen}
}
function rhCheckChampionshipDiscoveries(before){
 const s=rhSpace(),seen=s.championshipDiscoveries||(s.championshipDiscoveries={});
 const unlocked=rhEligibleChampionshipsForCars(s.cars||[]).filter(x=>!before.eligible.has(rhChampDiscoveryKey(x.type,x.value))&&!seen[rhChampDiscoveryKey(x.type,x.value)]);
 if(!unlocked.length)return;
 unlocked.forEach(x=>seen[rhChampDiscoveryKey(x.type,x.value)]=true);rhSave();
 rhShowChampionshipDiscovery(unlocked,0)
}
function rhShowChampionshipDiscovery(items,index){
 const x=items[index];if(!x)return;
 document.getElementById('rhChampDiscoveryOverlay')?.remove();
 document.body.insertAdjacentHTML('beforeend',`<div id="rhChampDiscoveryOverlay" class="rhOverlay" onclick="if(event.target===this)rhCloseChampDiscovery()"><div class="rhModal rhChampDiscoveryModal"><button class="rhModalX" onclick="rhCloseChampDiscovery()">×</button><div class="rhChampDiscoveryIcon">🏁</div><small>NEW CHAMPIONSHIP AVAILABLE</small><h2>${esc(x.name)}</h2><p>${esc(x.copy)}</p><div class="rhModalActions"><button class="btn secondary" onclick="rhCloseChampDiscovery()">NOT NOW</button><button class="btn" onclick="rhViewDiscoveredChampionship()">VIEW IN FESTIVAL</button></div></div></div>`);
 window.rhPendingChampDiscovery={items,index}
}
function rhCloseChampDiscovery(){
 const p=window.rhPendingChampDiscovery;document.getElementById('rhChampDiscoveryOverlay')?.remove();
 if(p&&p.items[p.index+1])rhShowChampionshipDiscovery(p.items,p.index+1);else window.rhPendingChampDiscovery=null
}
function rhViewDiscoveredChampionship(){
...
function rhRenameManufacturerPrompt(){
 ...
 rhOpenManufacturerRenameModal('',false)
}
function rhRenameManufacturerFromGarage(oldName){rhOpenManufacturerRenameModal(oldName,true)}
function rhOpenManufacturerRenameModal(oldName='',returnToGarage=false){
 const s=rhSpace(),makes=[...new Set((s.cars||[]).map(c=>c.make).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
 if(!makes.length)return toast('No Manufacturers to rename');
