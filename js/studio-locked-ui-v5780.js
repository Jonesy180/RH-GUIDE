...
window.rhDataBackups=function(){
 const s=rhSpace(),bs=(s.backups||[]).slice().reverse();show('more');q('more').innerHTML=`<div class="rhScene rhSettingsScene">${rhHeader('DATA / BACKUPS','Protect your OTG! data','settings','more')}</div><div class="rhContent rhConformance rhBackupsFinal">
 ...
 ${bs.length?bs.map((b,i)=>`<section class="rhBackupCard rhBackupCardFinal">
  ...
  <div class="rhBackupActions35"><button class="btn rhBackupRestoreFinal" onclick="rhRestoreDetail('${b.id}')">RESTORE</button><button class="btn dangerBtn rhBackupDelete35" onclick="rhDeleteBackupConfirm('${b.id}')">DELETE</button></div>
 </section>`).join(''):rhEmpty('NO SAVED BACKUPS','Create a backup to protect your OTG! data.','CREATE BACKUP','rhBackupFinal()')}</div>`;
};
...
window.rhDeleteBackupConfirm=function(id){const b=(rhSpace().backups||[]).find(x=>x.id===id);if(!b)return;rhConfirm({title:'DELETE THIS BACKUP?',copy:'This permanently deletes only the selected saved backup.',detail:`${b.spaceName} • ${new Date(b.date).toLocaleString('en-GB')}`,safeguard:'Your current OTG! Space, its racing data and all other Spaces will not be affected.',confirmLabel:'DELETE BACKUP',danger:true,onConfirm:`rhDeleteBackupFinal('${id}')`})};
...
window.rhOnboardingStep=function(step=1){
 ...
 else if(step===2)body=`<small>STEP 2 OF 3</small><h1>FAVOURITE MANUFACTURER</h1><p>Choose a Favourite Manufacturer for this OTG! Space.</p><input id="rhOnMakeSearch" placeholder="Search manufacturers" oninput="rhOnboardingFilterMakes(this.value)"><div id="rhOnMakeList" class="rhOnMakeList">${makes.map(m=>`<button data-make="${safe(m).toLowerCase()}" data-make-value="${encodeURIComponent(m)}" onclick="rhOnboardingSelectMake(decodeURIComponent(this.dataset.makeValue))">${safe(m)}</button>`).join('')}</div><button id="rhOnMakeContinue" class="btn rhPrimaryWide" disabled onclick="rhOnboardingConfirmMake()">SET FAVOURITE &amp; CONTINUE</button><button class="btn secondary" onclick="rhOnboardingStep(1)">BACK</button>`;
